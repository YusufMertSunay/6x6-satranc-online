...
// sohbetinde üst üste en fazla 3 mesaj atabilir. Bu, mesaj dizisinin
// SONUNDAN geriye doğru "aynı gönderen" sayılarak hesaplanıyor -- ayrı bir
// sayaç tutmaya gerek yok (bkz. _trailingStreak).
const MAX_CONSECUTIVE_PLAYER_CHAT_MSGS = 2;
const MAX_CONSECUTIVE_SPECTATOR_CHAT_MSGS = 3;
  ...
  sendChat(gameId, userId, text) {
    ...
    if (live && live.status === 'active') {
      ...
      if (this._trailingStreak(live.spectatorChat, userId) >= MAX_CONSECUTIVE_SPECTATOR_CHAT_MSGS) {
        const err = new Error('Başka biri araya mesaj yazana kadar üst üste en fazla 3 mesaj gönderebilirsin.');
    ...
    // birleşik sohbete yazılıyor, süre/sıra kısıtlaması YOK (kullanıcı
    // isteği sadece CANLI sohbetler için sınır istedi).
    const rec = this.store.getGame(gameId);
    if (!rec) throw new Error('Oyun bulunamadı.');
