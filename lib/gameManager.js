...
// Kullanıcı isteği: atılan hiçbir sohbet mesajı 280 karakteri geçemesin
// (Twitter/X'teki karakter sınırıyla aynı, kullanıcıya tanıdık gelsin diye).
...
class GameManager {
  ...
  sendChat(gameId, userId, text) {
    ...
    if (raw.length > CHAT_MESSAGE_MAX_LENGTH) throw new Error('Mesaj çok uzun (en fazla 280 karakter).');
    ...
    if (live && live.status === 'active') {
      const color = this.colorOf(live, userId);
      if (color) {
        const otherId = color === 'white' ? live.blackId : live.whiteId;
        // Kullanıcı isteği: "mesaj atmasını engellediğim kullanıcılarla
        // oynarkenki chatte, mesaj atmasını engellediğim kullanıcılar bana
        // mesaj atamaz". Burada userId gönderen, otherId rakip -- eğer
        // RAKİP beni (userId'yi) mesaj-engellemişse (isMessageBlocked(otherId,
        // userId): "otherId, userId'yi engellemiş"), gönderemem. Bu kontrol
        // SADECE canlı oyuncu sohbetinde var -- oyun bittikten sonraki
        // (merged/analiz) sohbette YOK, orada bunun yerine engellenen
        // kullanıcının mesajları silinmeden otomatik GİZLENİYOR (çünkü
        // messageBlockUser zaten otomatik olarak mutedUserIds'e de ekliyor,
        // bkz. store.js: messageBlockUser + _isMessageHiddenFor).
        //
        // ÖNEMLİ AYRIM (mevcut, ÖNCEDEN test edilmiş "komple engelleme"
        // (blockUser) davranışını BOZMAMAK için): blockUser da otomatik
        // olarak messageBlockedUserIds'e ekliyor (DM'lerin de engellenmesi
        // için, bkz. store.js), ama komple engellemenin canlı oyun
        // sohbetindeki ESKİ davranışı hep "gönderilsin ama engelleyene
        // GÖRÜNMESİN" şeklindeydi (bkz. test_block_rematch_challenge.js:
        // engel kaldırılınca gizli mesajların ZAMANA GÖRE doğru sırada geri
        // gelmesi bekleniyor -- yani mesaj her zaman GÖNDERİLMİŞ olmalı).
        // Bu yüzden burada SADECE "hafif" mesaj-engeli (komple engel OLMADAN
        // sadece-mesaj-engeli) varsa gönderimi reddediyoruz; komple
        // engellemede eskisi gibi mesaj gönderiliyor, sadece görüntülemede
        // gizleniyor (_isMessageHiddenFor zaten isBlocked'ı da kontrol ediyor).
        const lightlyMessageBlockedOnly = this.store.isMessageBlocked(otherId, userId) && !this.store.isBlocked(otherId, userId);
        if (lightlyMessageBlockedOnly) {
          const err = new Error('Rakibin seni mesajla engellemiş, bu oyunda ona mesaj gönderemezsin.');
          err.code = 'CHAT_BLOCKED_BY_OPPONENT';
          throw err;
        }
        ...
        // Kullanıcı isteği: "ben mesaj atarsam mesaj engeli otomatik olarak
        // kalkar" -- ben (userId) rakibi (otherId) daha önce SADECE mesajını
        // (komple değil) engellemişsem, ona BURADA (canlı oyun sohbetinde)
        // mesaj göndermem bu engeli otomatik kaldırır (sendDirectMessage'daki
        // AYNI kuralın oyun-içi sohbete genellemesi). Eğer aslında komple
        // engellemişsem (isBlocked de true), messageBlockedUserIds'i BURADA
        // temizlemiyoruz -- aksi halde komple engel hâlâ dururken (DM'lerin
        // engellenmesi gereken hâli) yanlışlıkla DM engeli kalkmış olurdu.
        if (this.store.isMessageBlocked(userId, otherId) && !this.store.isBlocked(userId, otherId)) {
          this.store.unMessageBlockUser(userId, otherId);
        }
  ...
  _finalizeGame(game, reason, winnerColor) {
    ...
    // Kullanıcı isteği: oyun bitince gösterilen "yeni puanın" yanında da
    // (puansız oyunlarda zaten bu satır hiç gösterilmiyor) bu kategoride
    // henüz 8 puanlı maç oynanmadıysa mavi "?" işareti çıksın diye -- bkz.
    // public/js/game.js: renderGameOverBanner. whiteUser/blackUser yukarıda
    // updateUserRating tarafından ZATEN güncellenmiş (aynı obje referansı),
    // bu yüzden burada okunan gamesPlayed sayısı bu oyunu da içeriyor.
    const whiteProvisionalAfter = whiteUser ? this.store.isProvisional(whiteUser, category) : false;
    const blackProvisionalAfter = blackUser ? this.store.isProvisional(blackUser, category) : false;
    ...
    this.store.saveFinishedGame(game.id, {
      ...
      whiteProvisionalAfter,
      blackProvisionalAfter,
    ...
    const payload = {
      ...
      whiteProvisionalAfter, blackProvisionalAfter,
    };
