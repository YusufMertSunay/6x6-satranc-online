...
class GameManager {
  ...
  // ============================================================
  // İKİ OYUNCU ARASINDAKİ "İLİŞKİ" DURUMU — kullanıcı isteği: bir kullanıcı
  // aynı hedefe art arda 2 kez teklif gönderip İKİSİ DE reddedilirse,
  // TEKLİFİ GÖNDEREN kişi o hedefe 24 saat boyunca yeni teklif gönderemez
  // VE bu süre boyunca ikisi hızlı eşleşmede de (tesadüfen dahi)
  // eşleştirilemez. Bu "hızlı eşleşme engeli" tek yönlü değildir (ikisini
  // de kapsar) ama HEDEF kişi kendi isteğiyle teklif gönderirse (rolleri
  // değiştirirse) hemen kalkar — teklifi 2 kez reddedilen kişinin 24
  // saatlik "teklif gönderememe" cezası ise DEĞİŞMEDEN devam eder. Eğer bu
  // ikinci (rol değişmiş) teklif dizisi de art arda 2 kez reddedilirse,
  // artık bu iki oyuncu 24 saat boyunca HİÇBİR ŞEKİLDE (ne özel davetle ne
  // hızlı eşleşmeyle, hiçbir yönde) eşleşemez.
  //
  ...
  _getPairRelation(a, b) {
    ...
    if (!rel) {
      ...
      // reddediliyor ve kaç kez üst üste. blockLevel: 0 = henüz hiç ceza
      // uygulanmadı, 1 = bir yönde "soğuma + geçici hızlı eşleşme engeli"
      // uygulandı, 2 = tam (çift yönlü, kalıcı süreli) engel uygulandı.
      rel = { lastDeclinedBy: null, declineStreak: 0, blockLevel: 0, offerCooldownUntil: {}, matchBlockUntil: 0, fullBlockUntil: 0 };
  ...
  canOffer(fromId, toId) {
    ...
    const now = Date.now();
    if (rel.fullBlockUntil > now) return { ok: false, code: 'FULLY_BLOCKED', retryAfterMs: rel.fullBlockUntil - now };
  ...
  canMatch(aId, bId) {
    ...
    const now = Date.now();
    if (rel.fullBlockUntil > now) return false;
    if (rel.matchBlockUntil > now) return false;
    return true;
  }
  ...
  // yasağı kalksın"). Karşı tarafın 24 saatlik "teklif gönderememe" cezası
  // buradan ETKİLENMEZ, kendi süresi dolana kadar aynen devam eder.
  ...
  _recordOfferDeclined(fromId, toId) {
    ...
    if (rel.declineStreak >= 2) {
      const now = Date.now();
      if (rel.blockLevel === 0) {
        // İlk kez art arda 2 ret: sadece gönderen kişi 24 saat boyunca bu
        // hedefe teklif gönderemez VE bu süre boyunca ikisi hızlı
        // eşleşmede de karşılaşmaz (hedef kendi teklifini gönderene kadar
        // — bkz. _recordOfferSent'teki istisna).
        rel.offerCooldownUntil[fromId] = now + PAIR_BLOCK_MS;
        rel.matchBlockUntil = now + PAIR_BLOCK_MS;
        rel.blockLevel = 1;
      } else {
        // Roller değiştikten SONRA da (ikinci kez) art arda 2 ret
        // yaşandı: artık iki taraf da, hiçbir şekilde, 24 saat boyunca
        // eşleşemez.
        rel.fullBlockUntil = now + PAIR_BLOCK_MS;
        rel.blockLevel = 2;
      }
      rel.declineStreak = 0;
    }
  ...
  // "zincirini" sıfırlar (halihazırda uygulanmış zamanlı cezalar/engeller
  // -- offerCooldownUntil, matchBlockUntil, fullBlockUntil -- kendi
  // sürelerini doldurana kadar AYNEN kalmaya devam eder; bu sadece YENİ
  // bir ceza tetiklenme riskini sıfırlar).
  ...
  offerRematch(gameId, userId) {
    ...
    if (!offerCheck.ok) {
      const err = new Error(offerCheck.code === 'FULLY_BLOCKED' ? 'Bu oyuncuyla şu anda hiçbir şekilde eşleşemezsin.' : 'Bu oyuncuya şu anda meydan okuyamazsın.');
  ...
  createChallenge(challengerId, targetUsername, timeControlKey, colorChoice, ranked) {
    ...
    // Art arda 2 kez reddedilen tekliflerden doğan 24 saatlik "soğuma" ya
    // da tam engel varsa (bkz. canOffer), yeni bir meydan okuma gönderilemez.
