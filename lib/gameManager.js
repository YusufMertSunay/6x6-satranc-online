...
// Doğrudan meydan okuma teklifleri bu süre içinde yanıtlanmazsa otomatik
// olarak geçersiz sayılır (lichess'teki gibi) — aksi halde biri çevrimdışı
// olduğunda ya da unuttuğunda teklif sonsuza kadar "bekliyor" görünürdü.
...
class GameManager {
  ...
  createChallenge(challengerId, targetUsername, timeControlKey, colorChoice) {
    if (!TIME_CONTROLS[timeControlKey]) throw new Error('Geçersiz süre kontrolü.');
    ...
    const expireTimer = setTimeout(() => {
      this._clearChallenge(id);
      this.hub.sendTo(challengerId, 'challenge_expired', { challengeId: id });
      this.hub.sendTo(target.id, 'challenge_expired', { challengeId: id });
    }, CHALLENGE_TTL_MS);
    // Node'un olay döngüsünü sadece bu zamanlayıcı yüzünden açık tutmasın.
    if (expireTimer.unref) expireTimer.unref();

    const challenge = { id, challengerId, targetId: target.id, timeControlKey, colorChoice, expireTimer };
  ...
  respondChallenge(challengeId, userId, accept) {
    ...
    this._createGame(whiteId, blackId, challenge.timeControlKey);
    return { ok: true };
  }
